...
import sys
import logging
from typing import Optional, Any, List # Added List and Any for type hints
...
def _setup_arg_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--llm-api-base-url", default=os.getenv("LLM_API_BASE_URL"),
    ...
    parser.add_argument(
        "--limit-user-prompt-length", type=int, default=100,
        help="Maximum allowed tokens in user prompt (default: 100)"
    )
    parser.add_argument(
        "--disable-logging", action='store_false', dest='enable_logging',
        help="Disable LLM usage logging (会计)."
    )
    parser.add_argument(
        "--disable-rate-limiting", action='store_false', dest='enable_rate_limiting',
        help="Disable rate limiting (currently a placeholder)."
    )
    parser.add_argument(
        "--disable-audit-log", action='store_false', dest='enable_audit_log',
        help="Disable audit logging of prompts and replies."
    )
    parser.add_argument(
        "--skip-outbound-key-leaks", action="store_true",
...
def _validate_allowed_models(args_model: str, allowed_models_file_path: Optional[str]) -> None:
    ...
    if args_model not in allowed_models:
        logger.warning(f"Model '{args_model}' is not in the allowed models list from {allowed_models_file_path}")
        sys.exit(1)
    logger.debug("Model '%s' validated against allowed models file '%s'.", args_model, allowed_models_file_path)
